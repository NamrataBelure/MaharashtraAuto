...
public class EngAssDao implements IEngAss {
	...
	public String AddEngAss(EngAssModel engass) {
		...
		try {
			...
			pre.setInt(1, engass.getUserId());
			pre.setInt(2, engass.getJobId());
			pre.setString(3, engass.getAssDate());
			pre.setInt(4, engass.getEsttimeTaken());
	...
	public List<EngAssModel> GetAssignments() {
		...
		try {
			...
			while (rs.next()) {
				...
				es.setUserId(rs.getInt("userId"));
				es.setJobId(rs.getInt("jobId"));
				es.setAssDate(rs.getString("assDate"));
				es.setEsttimeTaken(rs.getInt("esttimeTaken"));
